...
namespace nn::operations {
	Divide::Divide(std::shared_ptr<Tensor> a, std::shared_ptr<Tensor> b)
		: Operation(a, b) {	};
	...
	std::shared_ptr<Tensor> Divide::operation() {
		calculate();
		return c_;
	}

	void Divide::derivative(bool recursion) {
		auto tensor_a = a_->get_tensor(); 
		auto tensor_b = b_->get_tensor(); 
		...
		for (int i = 0; i < std::max(size_of_a, size_of_b); i++) {
			...
			double val_b = (*b_)[i % size_of_b];
			double val_a = (*a_)[i % size_of_a];
			a_->add_to_delta_grads(i % size_of_a, 1 / val_b * grad_c);
			b_->add_to_delta_grads(i % size_of_b, -val_a / (val_b * val_b) * grad_c);
			a_->add_to_iteration_grads(i % size_of_a, 1 / val_b * grad_c);
			b_->add_to_iteration_grads(i % size_of_b, -val_a / (val_b * val_b) * grad_c);
		}
	...
	void Divide::zero_child_grad(bool recursion) {
		a_->add_reference_calculated_();
		b_->add_reference_calculated_();
