...
namespace nn::operations {
	Multiply::Multiply(std::shared_ptr<Tensor> a, std::shared_ptr<Tensor> b)
		: Operation(a, b) {	};
	...
	std::shared_ptr<Tensor> Multiply::operation() {
		calculate();
		return c_;
	}

	void Multiply::derivative(bool recursion) {
		auto tensor_a = a_->get_tensor(); 
		auto tensor_b = b_->get_tensor(); 
		...
		for (int i = 0; i < std::max(size_of_a, size_of_b); i++) {
			...
			a_->add_to_delta_grads(i % size_of_a, tensor_b[i % size_of_b] * grad_c);
			b_->add_to_delta_grads(i % size_of_b, tensor_a[i % size_of_a] * grad_c);
			a_->add_to_iteration_grads(i % size_of_a, tensor_b[i % size_of_b] * grad_c);
			b_->add_to_iteration_grads(i % size_of_b, tensor_a[i % size_of_a] * grad_c);
		}
	...
	void Multiply::zero_child_grad(bool recursion) {
		a_->add_reference_calculated_();
		b_->add_reference_calculated_();
